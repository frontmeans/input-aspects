...
export abstract class InGroup<TModel extends object> extends InContainer<TModel> {
  ...
  protected _applyAspect<TInstance, TKind extends InAspect.Application.Kind>(
    ...
    return aspect === InGroup__aspect
      ? (inconvertibleInAspect(this, InGroup, this) as InAspect.Application.Result<
          TInstance,
          TModel,
          TKind
        >)
...
export namespace InGroup {
   ...
   */
  export type Entry<TModel, TKey extends keyof TModel = keyof TModel> = readonly [
    TKey,
    InControl<TModel[TKey]>,
  ];
...
export abstract class InGroupControls<TModel>
  ...
  implements
    EventSender<[InGroup.Entry<TModel>[], InGroup.Entry<TModel>[]]>,
   ...
   */
  abstract set<TKey extends keyof TModel>(
    key: TKey,
...
class InGroupSnapshot<TModel> implements InGroup.Snapshot<TModel>, PushIterable<InControl<any>> {
  ...
  [PushIterator__symbol](
    accept?: PushIterator.Acceptor<InControl<any>>,
  ): PushIterator<InControl<any>> {
  ...
  private newEntry<TKey extends keyof TModel>(
    ...
    return [
      ...
      new Supply(reason => {
        ...
        .needs(this._supply)
        .needs(supply)
        .whenOff(reason => supply.off(reason === inControlReplacedReason ? undefined : reason)),
    ];
  }
...
class InGroupControlControls<TModel extends object> extends InGroupControls<TModel> {
  ...
  private readonly _updates = new EventEmitter<
    [[keyof TModel, InGroupEntry][], [keyof TModel, InGroupEntry][]]
  >();
  ...
  constructor(private readonly _group: InGroupControl<TModel>) {
    ...
    this.on = this._updates.on.do(
      translateOn((send, added, removed) => {
        send(added.map(controlEntryToGroupEntry), removed.map(controlEntryToGroupEntry));
      }),
    );
 ...
 * @internal
 */
function controlEntryToGroupEntry<TModel extends object>([key, [control]]: [
  keyof TModel,
  InGroupEntry]): InGroup.Entry<TModel> {
  return [key, control];
}
...
class InGroupControl<TModel extends object> extends InGroup<TModel> {
  ...
  constructor(
    ...
    opts: {
      readonly aspects?:
        | InConverter.Aspect<TModel>
        | readonly InConverter.Aspect<TModel>[]
        | undefined;
    },
  ) {
    super(opts);
...
function readInGroupData<TModel extends object>({
  ...
  return afterAll(csData).do(
    mapAfter(controlsData => {
      const data: Partial<TModel> = { ...model };

      itsEach(
        overEntries(controlsData),
        <TKey extends keyof TModel>(
          keyAndControlData: readonly [keyof TModel, [InData.DataType<any>?]],
        ) => {
